      ...
      <div className="row">
        ...
        {peliculas.map((pelicula, index) => ( index < 12 &&
          <div className="col-lg-3 col-md-4 col-sm-4 col-xs-4 sol mb-2 mt-4" key={pelicula.id}> 
            <Pelicula imageSource={'https://www.themoviedb.org/t/p/w440_and_h660_face' + pelicula.poster_path} title={pelicula.title} id={pelicula.id}/>
          </div>
        ))}
      </div>
    </div>
...
function Pelicula({ imageSource, title, id }) {
